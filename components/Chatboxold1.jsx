...
const Chatbox = () => {
  ...
  const handleGenerateVectorStore = async () => {
    try {
      const res = await fetch('http://127.0.0.1:8000/generate-vector-store', {
        method: 'POST',
      });
      const data = await res.json();
      alert(data.message);
      ...
      alert("Failed to generate vector store.");
    }
  };
      ...
      <div className="flex flex-col items-center justify-end py-6 w-full bg-gray-100 text-white shadow-inner rounded-t-3xl">
        ...
        <div className="flex flex-col gap-4 w-[90%] items-center">
          ...
          <button
            ...
            className="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-xl text-lg transition"
          >
